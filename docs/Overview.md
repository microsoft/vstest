...
Microsoft.Testing.Platform (MTP) test applications are an emerging model. When the experimental `VSTEST_OPTIN_MTP` feature is enabled, the application hosts itself and TestPlatform drives discovery and execution over the MTP protocol instead of launching a VSTest testhost.
