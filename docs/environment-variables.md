...
- **Note**: xxHash128 is not supported yet. It ships to gather feedback and may change or be removed. It is intended to become the default in a later release, at which point the default of this variable flips to `0`. The meaning of each value stays the same, so setting `1` pins today's ids across that change.
