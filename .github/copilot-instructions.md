...
- After a snap, publishing from `main` breaks until dnceng adds the new `VS <version>` Maestro channel to the Arcade publishing configuration. Every official build of `main` fails at `Publish Using Darc` with `Channel with ID '<id>' is not configured to be published to.`
- A red `Publish Using Darc` on `main` within a couple of days of a `Branding as <version>` commit is this, **not** a regression. It clears with no change in this repository. Do not try to fix it here.
- The snap also leaves the new `rel/<version>` branch without a Maestro default channel. Those builds report **success** and publish nothing.
- Full checklist, with both failure modes and the exact `darc` commands: [docs/snap.md](../docs/snap.md).
