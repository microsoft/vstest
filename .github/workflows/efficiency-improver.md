...
- Micro-optimisations and marginal gains (single-digit-percent tweaks, hand-inlining, saving a handful of allocations off a cold path).
...
- **Discount big-N algorithmic wins unless you prove N is genuinely large on a common path.** An `O(n²) → O(n)` change over a per-test collection is only high-impact if that code really runs at n ≈ 1k–10k in common scenarios. At n = 1 (the most common run) it saves nothing — verify the real, common-case N before claiming impact.
