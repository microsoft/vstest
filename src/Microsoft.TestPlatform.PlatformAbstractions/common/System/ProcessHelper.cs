...
public partial class ProcessHelper : IProcessHelper
{
                                ...
                                cts.Token.Register(() =>
                                {
                                    try
                                    {
                                        if (!p.HasExited)
                                        {
                                            ...
                                            // grandchild keeps it hanging). Coordinate the kill with the marker so
                                            // the exit handler uses the short abort budget only when our kill wins
                                            // a race with the process exiting naturally.
                                            KillProcess(p);
                                        }
                                    }
                                    catch
    ...
    private void KillProcess(Process process)
        => KillProcess(process, static process => process.Kill());

    internal void KillProcess(Process process, Action<Process> killProcess)
    {
        lock (_deliberatelyTerminatedProcessesLock)
        {
            // Kill can race with a natural process exit. Keep the exit handler from reading the marker until
            // Kill reports its outcome, and publish the marker only after a successful kill. If Kill throws
            // because the process exited first, the exit remains unmarked and receives the crash drain budget.
            killProcess(process);
    ...
    internal bool WasDeliberatelyTerminated(Process process)
    {
        lock (_deliberatelyTerminatedProcessesLock)
        {
    ...
    public void TerminateProcess(object? process)
    {
        try
        {
            if (process is Process proc && !proc.HasExited)
            {
                // Coordinate the kill with its marker so a process that exits naturally between HasExited and
                // Kill is not misclassified as a deliberate termination.
                KillProcess(proc);
            }
        }
        catch (InvalidOperationException)
