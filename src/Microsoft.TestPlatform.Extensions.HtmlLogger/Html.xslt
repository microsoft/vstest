...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template name ="Results">
        <xsl:for-each select ="tp:TestResultCollection">
            ...
            <xsl:if test="tp:ResultCollectionListByClass/tp:TestResultByClass/tp:ResultList!=''">
                <div class ="list-row" onclick="ToggleClass('{$Source}')">
                    <xsl:value-of select = "tp:Source" />
                </div>
                <div class ="inner-row" Id="{$Source}" style="display:none;">


                    <xsl:for-each select ="tp:ResultCollectionListByClass/tp:TestResultByClass">
                        <xsl:variable name="Source2" select="tp:UniqueId" />
                        <div class ="list-row" onclick="ToggleClass('{concat($Source2,'-testResultList')}')">
                            <xsl:value-of select = "tp:ClassName" />
                        </div>
                        <div class ="inner-row" Id="{concat($Source2,'-testResultList')}" style="display:block;">
                            <xsl:for-each select ="tp:ResultList/tp:TestResult">
    ...
    <xsl:template name = "FailedResults">
        <xsl:for-each select ="tp:TestResultCollection">
            ...
            <xsl:variable name="Source" select="tp:Id" />
            <xsl:if test="tp:ResultCollectionListByClass/tp:TestResultByClass/tp:FailedResultList!=''">
                <div class ="list-row" onclick="ToggleClass('{concat($Source,'-testResultbyClass')}')">
                    <xsl:value-of select = "tp:Source" />
                </div>
                <div class ="inner-row" Id="{concat($Source,'-testResultbyClass')}" style="display:block;">
                    <xsl:for-each select ="tp:ResultCollectionListByClass/tp:TestResultByClass">
                        <xsl:variable name="Source2" select="tp:UniqueId" />

                        <div class ="list-row" onclick="ToggleClass('{concat($Source2,'-failedResultList')}')">
                            <xsl:value-of select = "tp:ClassName" />
                        </div>
                        <div class ="inner-row" Id="{concat($Source2,'-failedResultList')}" style="display:block;">
                            <xsl:for-each select ="tp:FailedResultList/tp:TestResult">
                                <xsl:call-template name ="TestResult"/>
                            </xsl:for-each>
                        </div>
                    </xsl:for-each>
    ...
    <xsl:template name="TestResult" match="tp:TestResult">
        ...
        <xsl:if test ="tp:InnerTestResults!=''">
            <div class ="row" onclick="ToggleClass('{concat($TestResultId,'-',name(..))}')">
                <xsl:call-template name = "Result" />
            </div>
            <a Id="{concat($TestResultId,'-',name(..))}" style="display:none;">
    ...
    <xsl:template match="tp:InnerTestResults">
        <div class ="inner-row">
            <xsl:apply-templates select = "tp:TestResult" />
        </div>
    </xsl:template>
    ...
    <xsl:template match = "tp:ErrorMessage">
        Error: <span class="error-message">
            <pre>
                <xsl:value-of select = "." />
            </pre>
        </span><br />
    ...
    <xsl:template match = "tp:ErrorStackTrace">
        Stack trace: <span class="error-stack-trace">
            <pre>
                <xsl:value-of select = "." />
            </pre>
        </span><br />
