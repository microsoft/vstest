...
public class ProxyOperationManager
{
    ...
    private void ThrowOnTestHostExited(IEnumerable<string> sources, bool testHostExited)
    {
        if (testHostExited)
        {
            ...
            throw new TestPlatformException(string.Format(CrossPlatEngineResources.TestHostExitedWithError, string.Join(", ", sources), _testHostProcessStdError));
        }
    }
    ...
    private void ThrowExceptionOnConnectionFailure(IEnumerable<string> sources, int connTimeout)
    {
        ...
        if (!StringUtils.IsNullOrWhiteSpace(_testHostProcessStdError))
        {
            ...
            errorMsg = string.Format(CrossPlatEngineResources.TestHostExitedWithError, string.Join(", ", sources), _testHostProcessStdError);
        }
