...
namespace Microsoft.TestPlatform.Extensions.BlameDataCollector
{
    ...
    public class ProcDumpProcessDumpUtility : IProcessDumpUtility
    {
        ...
        public ProcDumpProcessDumpUtility()
            : this(new ProcessHelper(), new FileHelper(), new PlatformEnvironment(), new NativeMethodsHelper())
        {
        }

        public ProcDumpProcessDumpUtility(IProcessHelper processHelper, IFileHelper fileHelper, IEnvironment environment, INativeMethodsHelper nativeMethodsHelper)
        {
        ...
        public void StartHangBasedProcessDump(int processId, string dumpFileGuid, string testResultsDirectory, bool isFullDump = false, string frameworkVersion = null)
        {
            ...
            if (t)
            {
                throw new NotSupportedException("hang dump should be done diferently");
            }
        ...
        {
            var here = Path.GetDirectoryName(typeof(ProcDumpProcessDumpUtility).Assembly.Location);
