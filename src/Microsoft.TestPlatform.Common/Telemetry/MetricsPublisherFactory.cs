// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT license. See LICENSE file in the project root for full license information.

namespace Microsoft.VisualStudio.TestPlatform.Common.Telemetry
{
    /// <summary>
    /// The Metrics Publisher factory.
    /// </summary>
    public class MetricsPublisherFactory
    {
        private bool telemetryOptedOut;

        /// <summary>
        /// Creates Instance of <see cref="MetricsPublisherFactory"/> class.
        /// </summary>
        /// <param name="isOptedOut">Telemetry opted out or not.</param>
        public MetricsPublisherFactory(bool isOptedOut)
        {
            this.telemetryOptedOut = isOptedOut;
        }

        /// <summary>
        /// Returns the Metrics Publisher.
        /// </summary>
        /// <returns>Returns the Metrics Publisher on basis of Telemetry opted in or not</returns>
        public IMetricsPublisher GetMetricsPublisher()
        {
            if (this.telemetryOptedOut)
            {
                return new NoOpMetricsPublisher();
            }

            return new MetricsPublisher();
        }
    }
}
