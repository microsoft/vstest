...
public class ProcessHelperTests
{
    ...
    [TestMethod]
    public void KillProcessShouldNotMarkNaturalExitWhenKillLosesRace()
    {
        ...
        Assert.ThrowsExactly<InvalidOperationException>(
            () => processHelper.KillProcess(
                process,
                _ => throw new InvalidOperationException("The process exited naturally before Kill.")));

        Assert.IsFalse(
            processHelper.WasDeliberatelyTerminated(process),
            "A natural exit that wins the race with Kill must retain the generous crash drain budget.");
    }
}
