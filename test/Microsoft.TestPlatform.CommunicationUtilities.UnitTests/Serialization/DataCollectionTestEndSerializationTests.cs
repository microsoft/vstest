...
/// <summary>
/// Wire-format tests for <see cref="MessageType.DataCollectionTestEnd"/>
/// ("DataCollection.TestEnd").
///
/// This message is sent by the test execution engine to data collectors when a test case ends.
/// The payload is <see cref="TestCaseEndEventArgs"/> containing the test case metadata,
/// data collection context, and test outcome.
///
...
public class DataCollectionTestEndSerializationTests
{
    // ── Example payload ──────────────────────────────────────────────────
    private static readonly TestCaseEndEventArgs Payload = BuildPayload();

    // ── V1 wire format ───────────────────────────────────────────────────
    // Protocol versions 0, 1, 3 — TestCase uses Properties array format.
    private static readonly string V1Json = """
        {
          "MessageType": "DataCollection.TestEnd",
          "Payload": {
            "TestOutcome": 1,
    ...
    public void SerializePayloadV1()
    {
        var json = JsonDataSerializer.Instance.SerializePayload(
            MessageType.DataCollectionTestEnd, Payload, version: 1);
    ...
    {
        var json = JsonDataSerializer.Instance.SerializePayload(
            MessageType.DataCollectionTestEnd, Payload, version: 7);
    ...
    public void DeserializePayloadV1()
    {
        ...
        var result = JsonDataSerializer.Instance.DeserializePayload<TestCaseEndEventArgs>(message);
    ...
    {
        var json = JsonDataSerializer.Instance.SerializePayload(
            MessageType.DataCollectionTestEnd, Payload, version);
        var message = JsonDataSerializer.Instance.DeserializeMessage(json);
        var result = JsonDataSerializer.Instance.DeserializePayload<TestCaseEndEventArgs>(message);

        Assert.IsNotNull(result);
        Assert.AreEqual(TestOutcome.Passed, result.TestOutcome);
    ...
    private static TestCaseEndEventArgs BuildPayload()
    {
        var tc = new TestCase(
        ...
        return new TestCaseEndEventArgs(tc, TestOutcome.Passed);
    }

    private static void AssertPayloadFields(TestCaseEndEventArgs? result)
    {
        Assert.IsNotNull(result);
        Assert.AreEqual(TestOutcome.Passed, result.TestOutcome);
        Assert.AreEqual(new Guid("a1b2c3d4-e5f6-7890-abcd-ef1234567890"), result.TestCaseId);
