...
/// <summary>
/// Wire-format tests for <see cref="MessageType.StopTestSessionCallback"/> ("TestSession.StopTestSessionCallback").
///
/// This message is sent as a callback after a test session has been stopped.
/// The payload is <see cref="StopTestSessionAckPayload"/> which contains the event args
/// with the session info, stop status, and metrics.
///
...
public class StopTestSessionCallbackSerializationTests
{
    // ── Example payload ──────────────────────────────────────────────────
    private static readonly StopTestSessionAckPayload Payload = BuildPayload();

    private static StopTestSessionAckPayload BuildPayload()
    {
        ...
        return new StopTestSessionAckPayload
        {
            EventArgs = new StopTestSessionCompleteEventArgs(sessionInfo)
            {
                IsStopped = true,
    ...
    private static readonly string V1Json = """
        {
          "MessageType": "TestSession.StopTestSessionCallback",
          "Payload": {
            "EventArgs": {
              "TestSessionInfo": {
    ...
    public void DeserializePayloadV1()
    {
        ...
        var result = JsonDataSerializer.Instance.DeserializePayload<StopTestSessionAckPayload>(message);
        ...
        Assert.IsTrue(result.EventArgs.IsStopped);
    ...
    {
        var json = JsonDataSerializer.Instance.SerializePayload(
            MessageType.StopTestSessionCallback, Payload, version);
