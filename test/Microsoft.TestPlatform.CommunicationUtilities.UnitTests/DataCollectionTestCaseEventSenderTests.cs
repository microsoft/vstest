...
public class DataCollectionTestCaseEventSenderTests
{
    ...
    public void SendTestCaseEndShouldReturnAttachments()
    {
        ...
        _mockCommunicationManager.Setup(x => x.ReceiveMessage()).Returns(new Message() { MessageType = MessageType.DataCollectionTestEndResult, Payload = JsonSerializer.SerializeToElement(new Collection<AttachmentSet>() { attachmentSet }) });
